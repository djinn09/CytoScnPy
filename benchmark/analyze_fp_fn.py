...
def match_items(finding_key, truth_keys):
    ...
    for t_key in truth_keys:
        ...
        # Path match (endswith)
        if not (f_path.endswith(t_path) or t_path.endswith(f_path) or 
                Path(f_path).name == Path(t_path).name):
            continue
            
        # Type match (method<->function equivalence)
        if not (f_type == t_type or 
                (f_type == "method" and t_type == "function") or
                (f_type == "function" and t_type == "method")):
            continue
