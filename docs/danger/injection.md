...
| **CSP-D105** | `assert` used in production             | LOW          | Asserts can be optimized away | Use explicit `if ...: raise`             |
| **CSP-D106** | `Jinja2 Environment(autoescape=False)`  | HIGH         | XSS vulnerability             | Set `autoescape=True`                    |
| **CSP-D107** | `django.utils.safestring.mark_safe`     | **MEDIUM**   | XSS bypass                    | Avoid unless content is strictly trusted |
